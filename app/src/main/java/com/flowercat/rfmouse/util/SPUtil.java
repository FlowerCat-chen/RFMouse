...
public class SPUtil {
	...
	public static final String APPS_KEY = "savedappmouse";
	
    public static final String FIRST_RUN_KEY = "is_first_run_rf";
    public static SharedPreferences sharedPreferences;

	public static String KEY_ACTIONS_DATA = "keys_added";
	...
	public static final String KEY_MGR_SHORT= "mgr_short";
	public static final String KEY_MGR_LONG= "mgr_long";
	public static final String KEY_MGR_SCROLL= "mgr_scroll_time";
	public static final String KEY_MGR_UD_DIS= "mgr_up_down_dis";
	public static final String KEY_MGR_LR_DIS= "mgr_left_right_dis";
	...
	public static final String KEY_ALIVE_SERVICE = "alive_service";
    ...
    public static void init(Context context) {
        if (sharedPreferences == null) {
            sharedPreferences = context.getApplicationContext()
                .getSharedPreferences("rfmouse", Context.MODE_MULTI_PROCESS);
        }
    }
	...
	// 将对象序列化为字符串
    public static String mtoString(Serializable o) throws IOException {
	...
	/**
     * 从SharedPreferences加载保存的应用列表
     */
    public static List<MouseOverSettingActivity.AppInfo> loadAppList(Context mCtx,List<MouseOverSettingActivity.AppInfo> appList) {
        appList.clear();
        String jsonString = getString(APPS_KEY, "[]");
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                // 使用 optString 和 optInt 避免因数据损坏导致的崩溃
                String name = jsonObject.optString("name", "未知应用");
                ...
                try {
                    // 验证应用是否仍然存在（可能已被卸载）
                    ApplicationInfo appInfo = mCtx.getPackageManager().getApplicationInfo(packageName, 0);
                    Drawable icon = mCtx.getPackageManager().getApplicationIcon(appInfo);
                    MouseOverSettingActivity.AppInfo app = new MouseOverSettingActivity.AppInfo(name, packageName, icon, mode);
                    appList.add(app);
                } catch (PackageManager.NameNotFoundException e) {
                    // 如果应用已卸载，跳过不添加
                    Log.w(TAG, "应用已卸载，跳过加载：" + packageName);
                    continue;
                }
            }
        } catch (JSONException e) {
            // 异常处理：JSON数据格式错误
            Log.e(TAG, "加载应用列表时发生JSON错误", e);
            Toast.makeText(mCtx, "加载列表失败，数据已损坏", Toast.LENGTH_LONG).show();
        }
    ...
    /**
     * 将当前应用列表保存到SharedPreferences
     */
    public static  void saveAppList(List<MouseOverSettingActivity.AppInfo> appList) {
        try {
            JSONArray jsonArray = new JSONArray();
            for (MouseOverSettingActivity.AppInfo app : appList) {
                JSONObject jsonObject = new JSONObject();
                jsonObject.put("name", app.getName());
                jsonObject.put("packageName", app.getPackageName());
                jsonObject.put("mode", app.getMode());
                jsonArray.put(jsonObject);
            }

            SharedPreferences.Editor editor = sharedPreferences.edit();
            editor.putString(APPS_KEY, jsonArray.toString());
            editor.apply(); // 使用 apply() 异步保存，性能更优
        } catch (JSONException e) {
            // 异常处理：保存时JSON数据出错
            Log.e(TAG, "保存应用列表时发生JSON错误", e);
        }
    }
