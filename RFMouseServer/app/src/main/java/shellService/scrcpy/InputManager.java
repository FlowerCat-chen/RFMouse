...
public final class InputManager {
    ...
    public boolean injectInputEvent(InputEvent inputEvent, int mode) {
        try {
                ...
                if (cause instanceof SecurityException) {
                    ...
                    if (message != null && message.contains("INJECT_EVENTS permission")) {
                        ...
                        if (lastPermissionLogDate <= now - 3000) {
                            EasyLog.logToFile(message);
                            EasyLog.logToFile("Make sure you have enabled \"USB debugging (Security Settings)\" and then rebooted your device.");
            ...
            EasyLog.logToFile("Could not invoke method:" + e.toString());
    ...
    public static boolean setDisplayId(InputEvent inputEvent, int displayId) {
        try {
            ...
            EasyLog.logToFile("Cannot associate a display id to the input event" + e.toString());
            return false;
        }
    }
    ...
    public static boolean setActionButton(MotionEvent motionEvent, int actionButton) {
        try {
            ...
            EasyLog.logToFile("Cannot set action button on MotionEvent" + e.toString());
            return false;
        }
    }
    ...
    public void addUniqueIdAssociationByPort(String inputPort, String uniqueId) {
        try {
            ...
            EasyLog.logToFile("Cannot add unique id association by port" + e.toString());
        }
    }
    ...
    public void removeUniqueIdAssociationByPort(String inputPort) {
        try {
            ...
            EasyLog.logToFile("Cannot remove unique id association by port" + e.toString());
        }
    }
}
