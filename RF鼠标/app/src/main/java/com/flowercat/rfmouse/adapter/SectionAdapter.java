...
public class SectionAdapter extends ArrayAdapter<SectionItem> {
    ...
    public SectionAdapter(Context context, int resource, List<SectionItem> objects) { 
        ...
        // 将 12dp 转换为对应的像素值，适配多种分辨率
        this.cornerRadiusPx = 12 * displayMetrics.density; 
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (sectionItem != null) {
			...
			if (Build.VERSION.SDK_INT >= 16) {
				holder.titleTextView.setBackground(shapeDrawable); // 4.1+直接用
			} else {
				holder.titleTextView.setBackgroundDrawable(shapeDrawable); // 4.0及以下兼容
			}
